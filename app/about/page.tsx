...
export default function AboutPage() {
    ...
    return (
        <div className={`pt-16 min-h-screen px-6 py-10 ${containerClass}`}>
            ...
            {/* Summary */}
            <section
                className="max-w-5xl mx-auto mt-12 fade-in-up"
            ...
            >
                ...
                <div className="mt-6 grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6">
                    {userData.about.skills.map((skill, index) => (
                        <div
                            key={index}
                            className={`p-4 rounded-lg ${cardBg} shadow-lg hover:shadow-xl transition`}
                        >
            ...
            >
                ...
                <div className="mt-6 space-y-6">
                    {userData.workExperience.map((exp, index) => (
                        <div
                            key={index}
                            className={`p-4 rounded-lg ${cardBg} shadow-lg transition`}
                        >
                            <h3 className="text-xl font-semibold">{exp.role}</h3>
                            <p className={`text-sm ${cardTextSmall}`}>
                                {exp.company} | {exp.period}
                            </p>
                            <ul
                                className={`mt-2 list-disc list-inside text-sm ${cardText}`}
                            >
